...
def parse_graphml(input_file_name, nx_topology, defa_node_type="OSHI-CR", defa_link_type="core"):
	...
	for e in edge_set:
		...
		#Carico il link 
		nx_topology.add_edge(int(src_id),int(dst_id), capacity = round(float(id_node_link_speed_dict[i])), allocated=0, flows=[])
		
		#GENERA COLLEGAMENTI CONTRARI A QUELLI SOPRA IN MODO DA CREARE LINK BIDIREZIONALI TRA I NODI CORE        
		nx_topology.add_edge(int(dst_id),int(src_id), capacity = round(float(id_node_link_speed_dict[i])), allocated=0, flows=[])
